...
class MainFrame(ctk.CTkFrame):
    def __init__(self, parent: "App"):
        ...
        label = ctk.CTkLabel(self, text="Insira suas credenciais", font=ctk.CTkFont(size=24))
        label.pack(pady=50, padx=10)

        # Create the text box for taking
        # username input from user
        self.user_entry = ctk.CTkEntry(self, placeholder_text="Nome", width=240)
        self.user_entry.pack(pady=8, padx=10)

        # Create a text box for taking
        # password input from user
        self.user_pass = ctk.CTkEntry(self, placeholder_text="Senha", show="*", width=240)
        self.user_pass.pack(pady=8, padx=10)

        # Create a login button to login
        self.button = ctk.CTkButton(self, text="Entrar", command=self.login, width=240)
        self.button.pack(pady=8, padx=10)

    def login(self):
        session = DB.get_session()

        stmt = select(User).where(User.name == self.user_entry.get()).where(User.password == self.user_pass.get())
